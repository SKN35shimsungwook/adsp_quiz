...
# 실제 ADsP 시험 기준: 총 50문항(1과목10·2과목10·3과목30), 문항당 2점, 과목별 40% 미만이면 과락
...
st.markdown(
    ...
    <style>
    :root{
      --ink:#2A2320; --ink-soft:#5B5048; --paper:#FBF8F4; --card:#FFFFFF;
      --accent:#8A4429; --accent-soft:#F3E7DE; --line:#ECE3D9;
    ...
    .stButton > button{
      ...
      box-shadow:0 1px 2px rgba(74,52,33,.04), 0 4px 14px rgba(74,52,33,.05);
    }
...
@st.cache_data
def get_concepts():
    """개념형 문제 은행에서 core_id(고유 개념)당 하나씩만 뽑은 215개 대표 개념 목록.
...
with st.sidebar:
    ...
    st.caption("데이터분석 준전문가 자가 학습 · 4지선다")
...
if ss.nav == "퀴즈":
    if ss.queue is None:
        ...
        with st.container(border=True):
            st.markdown("**실전 시험 모드** — 실제 ADsP 출제 기준(1과목 10·2과목 10·3과목 30, 총 50문항)으로 합격/과락 여부까지 진단합니다.")
            if st.button("실전 시험 모드 시작 (50문항)", type="primary", width="stretch"):
        ...
        if ss.exam_mode:
            ...
            if overall_pass:
                st.markdown(
                    ...
                    f'총점 {total_score}점 / 100점 (정답 {ss.correct_count}/{total})</div>',
                    unsafe_allow_html=True,
                )
            else:
                reason = []
                ...
                st.markdown(
                    ...
                    f'총점 {total_score}점 / 100점 (정답 {ss.correct_count}/{total})<br>'
                    f'{" · ".join(reason)}</div>',
                    unsafe_allow_html=True,
                )
...
# ============ 개념노트 (카드 / 노트 / OX 퀴즈) ============
    ...
    note_subject = st.radio("과목", ["전체", "1과목", "2과목", "3과목"], horizontal=True, key="concept_subject")
    if note_subject == "전체":
        note_subjects = [1, 2, 3]
    else:
